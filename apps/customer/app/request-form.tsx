    ...
    <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
      <Card className="space-y-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          {issueOptions.map((item) => (
            <button
              className={issueDescription === item.text ? "rounded-md bg-red-600 px-3 py-3 text-left text-xs font-black text-white shadow-lg shadow-black/30" : "rounded-md bg-white/10 px-3 py-3 text-left text-xs font-black text-red-100 ring-1 ring-white/10"}
              key={item.label}
        ...
        <div className="rounded-lg border border-white/10 bg-[#090b10] p-3">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="rounded-md bg-red-500/15 px-2 py-1 text-xs font-black text-red-200">WhatsApp handoff</span>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="space-y-2">
            ...
            <select className="min-h-12 w-full rounded-md border border-white/10 bg-[#090b10] px-3 font-semibold text-white outline-none focus:border-red-500" onChange={(event) => setServiceType(event.target.value as ServiceType)} value={serviceType}>
          ...
          <label className="space-y-2">
            ...
            <select className="min-h-12 w-full rounded-md border border-white/10 bg-[#090b10] px-3 font-semibold text-white outline-none focus:border-red-500" onChange={(event) => setBikeCategory(event.target.value as BikeCategory)} value={bikeCategory}>
              <option>100cc</option>
              <option>150cc</option>
          ...
          <label className="space-y-2">
            ...
            <select className="min-h-12 w-full rounded-md border border-white/10 bg-[#090b10] px-3 font-semibold text-white outline-none focus:border-red-500" onChange={(event) => setDistanceSlab(event.target.value as DistanceSlab)} value={distanceSlab}>
          ...
          <label className="space-y-2">
            ...
            <input className="min-h-12 w-full rounded-md border border-white/10 bg-[#090b10] px-3 font-semibold text-white outline-none placeholder:text-zinc-600 focus:border-red-500" onChange={(event) => setWhatsappNumber(event.target.value)} placeholder="9876543210" value={whatsappNumber} />
          </label>
        </div>
        <label className="space-y-2">
          <span className="text-sm font-semibold">Issue and pickup address</span>
          <textarea className="min-h-24 w-full rounded-md border border-white/10 bg-[#090b10] px-3 py-2 font-semibold text-white outline-none placeholder:text-zinc-600 focus:border-red-500" onChange={(event) => setPickupAddress(event.target.value)} placeholder="Pickup address" value={pickupAddress} />
        </label>
        <textarea className="min-h-20 w-full rounded-md border border-white/10 bg-[#090b10] px-3 py-2 font-semibold text-white outline-none placeholder:text-zinc-600 focus:border-red-500" onChange={(event) => setIssueDescription(event.target.value)} placeholder="Issue description" value={issueDescription} />
        ...
        {message ? <p className="rounded-md bg-white/10 p-3 text-sm font-semibold text-zinc-100">{message}</p> : null}
      </Card>
      <Card className="overflow-hidden p-0">
        ...
        <div className="grid grid-cols-3 gap-2 p-3 text-center text-xs font-black">
          <div className="rounded-md bg-red-50 p-2 text-red-700">Dispatch ETA</div>
